...
module.exports = (env) => {
  ...
  // define env files from file
  const platformFolder = platform === 'android' ? 'Android' : 'IOS';
  const envFileVars = require(env.vars ? `./app/settings/${env.vars}.json` : './app/settings/local.json');
  if (typeof envFileVars === 'object') {
    for (const prop in envFileVars) {
      envFileVars[prop] = JSON.stringify(envFileVars[prop]);
    }
  }

  const definePluginVars = Object.assign(envFileVars, {
  ...
  const config = {
    ...
    plugins: [
      ...
      new CopyWebpackPlugin([{
        ...
        {
          from: `${appResourcesFullPath}/${platformFolder}/src/config/${env.vars ? env.vars : 'local'}/**/*`,
          to: `${appResourcesFullPath}/${platformFolder}`,
          flatten: true,
          force: true,
        },
      ]),
